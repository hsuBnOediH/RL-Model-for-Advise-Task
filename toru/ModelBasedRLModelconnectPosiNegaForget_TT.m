function [results] = ModelBasedRLModelconnectPosiNegaForget_TT(task, MDP, params, sim)
...
for t = 1:task.num_trials
    ...
    if selected ~= 1
        ...
        % ---- connected update in start state ----

        if o==1
           pW(selected,1)  = pW(selected,1)  + params.eta_d_win  * (1 - pW(selected,1));
           pW(opposite,1)  = pW(opposite,1)  + params.eta_d_win  * (0 - pW(opposite,1));
        else
           pW(selected,1)  = pW(selected,1)  + params.eta_d_loss * (0 - pW(selected,1));
           pW(opposite,1)  = pW(opposite,1)  + params.eta_d_loss * (1 - pW(opposite,1));
        end
        ...
        if FORGETtoZero
            ...
            p_target = pW_init(2,2);  
        end

        if pW(2,2) >= 0.5
            omega = params.omega_a_posi;
        else
            omega = params.omega_a_nega;
        end

        pW(2,2) = pW(2,2) + omega*(p_target - pW(2,2));
        pW(2,2) = clip01(pW(2,2));

        pW(3,2) = 1 - pW(2,2);   % oppose in LH
        pW(2,3) = pW(3,2);       % oppose in RH
        pW(3,3) = pW(2,2);       % follow in RH
        ...
        % ---- connected update in hinted state ----
        if o==1
            pW(secchoice,hint)    = pW(secchoice,hint)    + params.eta_a_win  * (1 - pW(secchoice,hint));
            pW(secopposite,hint)  = pW(secopposite,hint)  + params.eta_a_win  * (0 - pW(secopposite,hint));
        else
            pW(secchoice,hint)    = pW(secchoice,hint)    + params.eta_a_loss * (0 - pW(secchoice,hint));
            pW(secopposite,hint)  = pW(secopposite,hint)  + params.eta_a_loss * (1 - pW(secopposite,hint));
        end
        ...
        if UPDATE_START_DIRECT_CHOICE
         ...
         if o==1
            ...
            pW(secopposite,1) = pW(secopposite,1) + params.eta_d_win  * (0 - pW(secopposite,1));
         else
            pW(secchoice,1)   = pW(secchoice,1)   + params.eta_d_loss * (0 - pW(secchoice,1));
            pW(secopposite,1) = pW(secopposite,1) + params.eta_d_loss * (1 - pW(secopposite,1));
         end
