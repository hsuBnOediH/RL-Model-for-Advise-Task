...
function [MDP] = spm_MDP_VB_X_advice_no_message_passing(MDP,OPTIONS)
...
if size(MDP,2) > 1
    ...
    for i = 1:size(MDP,2)
        ...
        %------------------------------------------------------------------
        OUT(:,i) = spm_MDP_VB_X_advice_no_message_passing(MDP(:,i),OPTIONS);
...
try, omega_context = MDP(1).omega_context; catch, omega_context = 1;end
...
for t = 1:T
    ...
    for m = M(t,:)
        ...
        if isfield(MDP,'link')
            ...
            %==============================================================
            MDP(m).mdp(t) = spm_MDP_VB_X_advice_no_message_passing(mdp);
        ...
        if ~HMM || T == t
                    ...
                    for j = 1:S         % loop over future time points
                        ...
                        for f = 1:Nf(m)
                            ...
                            if dF > exp(-8) || i > 4
                                ...
                                if j < R
                                    posterior = 0.5*px_forward + 0.5*px_backward + qL;
                                else
                                    posterior = px_forward + qL;
                                end
            ...
            if Np(m) > 1
                ...
                if t > 1
                    ...
                    num_columns = Np(m);
                    for j = 1:Nf(m)                       
                        previous_action = MDP(m).u(j,t-1);
                        for i = 1:num_columns
                            mask = V{m}(t-1,i,j) ~= previous_action;
                            if mask ~= 0
                                Q(i) = intmin('int32');
                            end
                        end
                ...
                for i = 1:1
                    ...
                    %------------------------------------------------------
                    qu = spm_softmax(qE{m}(p{m}) + Q(p{m}));
                    pu = spm_softmax(qE{m}(p{m}) + Q(p{m}));
    ...
    for t = 2
        ...
        if isfield(MDP,'a')
            for g = 1
                da     = O{m}(g,t);
                 for  f = 1:Nf(m)
                     %NOTE THAT I AM TAKING POSTERIOR OVER STATES FROM TIME
                     %3 and not t because we are not doing message passing
                     %so the belief from t=3 would not propagate back to t=2
                     da = spm_cross(da,X{m,f}(:,t));
                 end

                % if the message isn't propagating back from t=3 to t=2,
                % just take the belief form t=3
           
                % first state factor 
%                 da = spm_cross(da,X{m,1}(:,3));
%                 % second state factor
%                 da = spm_cross(da,X{m,2}(:,2));
                
                % zero out counts that were added to modalities of no
                % interest
                da     = da.*(MDP(m).a{g} > 0);
%                 MDP(m).a{g}(:,:,2) = MDP(m).a{g}(:,:,2)*omega + da(:,:,2)*;
                ...
                % only learn/forget when the advisor was chosen
...
%                  end
    ...
    if isfield(MDP,'d')
        ...
        %MDP(m).d{f} = (MDP(m).d{f} - MDP(m).d_floor)*omega_context + MDP(m).d_floor + X{m,f}(i,1)*eta;
        % we update concentration param for context based on what we see at
        % t = 3, hence X{m,f}(i,3)
