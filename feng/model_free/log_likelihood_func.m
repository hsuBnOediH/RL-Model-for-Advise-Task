function L = log_likelihood_func(P, M, U, Y)
    ...
    for i = 1:num_trials
        ...
        if mod(i, 30) == 1
            ...
            q_model.q_table(1, 1) = 40 * left_better + (-party_size)* (1-left_better);
            q_model.q_table(1, 2) = 40 * (1-left_better) + (-party_size) * left_better;
            % truthness * value win + (1-truthness) * value lose
            q_model.q_table(1, 3) = 20 * advise_truthness + (-party_size) * (1-advise_truthness);
            q_model.q_table(2, 1) = 20 * advise_truthness + (-party_size) * (1-advise_truthness);
            q_model.q_table(2, 2) = 20 * (1-advise_truthness) + (-party_size) * advise_truthness;
            q_model.q_table(2, 3) = NaN;
            q_model.q_table(3, 1) = 20 * (1-advise_truthness) + (-party_size) * advise_truthness;
            q_model.q_table(3, 2) = 20 * advise_truthness + (-party_size) * (1-advise_truthness);
        ...
        if actual_actions(1) == 3
            ...
            % (start,left/right) = (start,left/right) + fr * (advise_truthness - (start,left/right))
        ...
        if length(actual_actions) > 1
            ...
            if second_action == 1
                ...
                % update the (advise_left,left) or (advise_right,left) pair
                ...
                % connect update the q value for the (start,left) pair
                ...
                q_model.q_table(after_advice_state,2) = q_model.q_table(after_advice_state,2) + fr * (advise_truthness - q_model.q_table(after_advice_state,2));
                 ...
                 % if the subject chose right at time step 2
                if actual_reward > 0
                    % determine the which lr, fr to use
                    reward_term = params.reward_sensitivity * actual_reward;
                    lr = params.with_advice_win_learning_rate;
                ...
                % update the (advise_left,right) or (advise_right,right) pair
                ...
                % connect update the q value for the (start,right) pair
